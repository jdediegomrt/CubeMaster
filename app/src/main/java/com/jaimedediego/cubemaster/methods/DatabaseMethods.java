...
public class DatabaseMethods {
    ...
    public List<Detail> getTimesDetail(String puzzle, int mode) {
        ...
        switch (mode) {
            case 1:
                c = DatabaseMethods.getInstance().makeQuery("select time, date, scramble, num_solve from times where user_id=" + Session.getInstance().currentUserId + " and puzzle_id in (select id from puzzles where name='" + puzzle + "') order by num_solve asc");
                break;
            case 2:
                c = DatabaseMethods.getInstance().makeQuery("select time, date, scramble, num_solve from times where user_id=" + Session.getInstance().currentUserId + " and puzzle_id in (select id from puzzles where name='" + puzzle + "')");
                break;
            case 3:
                c = DatabaseMethods.getInstance().makeQuery("select time, date, scramble, num_solve from times where user_id=" + Session.getInstance().currentUserId + " and puzzle_id in (select id from puzzles where name='" + puzzle + "')");
                break;
            default:
                c = DatabaseMethods.getInstance().makeQuery("select time, date, scramble, num_solve from times where user_id=" + Session.getInstance().currentUserId + " and puzzle_id in (select id from puzzles where name='" + puzzle + "') order by num_solve desc");
                break;
        }
        if (c.moveToFirst()) {
            do {
